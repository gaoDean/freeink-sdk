...
| **Xteink Mallorn** | ESP32-S3 | SSD1677, UC8179, **or UC8279** (per batch) | 800×480 B/W | resistor-ladder buttons (4+2, X4-style), brightness/color-select frontlight, active-low LED-boost enable, native 4-bit SDMMC, TP4054 battery on an ADC; controller auto-detected at boot |
...
| `-DFREEINK_DEVICE_MALLORN` | Xteink Mallorn (S3, SSD1677/UC8179/UC8279 auto-detect + X4 button ladder + color-select frontlight + 4-bit SDMMC) |
...
| `-DFREEINK_SD_SDMMC=1` | use the native SDMMC backend — 4-bit or 1-bit per profile (needs `-DUSE_BLOCK_DEVICE_INTERFACE=1`); auto-on for de-link, X4 Pro, Paper Mono, and Mallorn |
...
`delink`, `murphy`, `m5paper_v11`, `sticky`, `x4pro`, `mallorn`, `papermono`) wired with the
